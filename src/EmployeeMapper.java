...
public class EmployeeMapper extends Mapper<LongWritable,Text, Text, Text>{
	...
	public void setup(Context context) throws IOException
	{
        ...
        while((line = br.readLine())!=null)
        {
                ...
                //splits[0] is the CustID no and splits[1 ] is the name
	...
	public void map(LongWritable key, Text val,Context context) throws IOException,InterruptedException
	{
		...
		/*
		 * customer Id is joining key in this data sets, 
		 * join the data based on the customer Id
		 */
		int count=0;
		long sum=0;
		if(CustMap.containsKey(splits[0]))   // only those transactions details which are available in customers 
			
		{
			 String Transtions=splits[3].replaceAll("[\\[ \\]]", "");   //replace []
            
			String []trans=Transtions.split(";");
			for(int i=0;i<trans.length;i++){
				count++;
				sum=sum+Integer.parseInt(trans[i].trim());
			}
			context.write(new Text(CustMap.get(splits[0])), new Text("Number of Trans:"+count+","+"Total Amount:"+sum)); // we can use custom input format class
		}
